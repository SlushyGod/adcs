...
#define CLOSE_BUMP    51  // Stops the doors from closing.
#define R_OPEN_BUMP   52  // Stops the right door from opening.
#define L_OPEN_BUMP   53  // Stops the left door from opening.
...
void doorWaitOpen(){
  ...
  */
  while (1)
  {
    if (digitalRead(R_OPEN_BUMP) == HIGH) digitalWrite(R_DOOR_OPEN,OFF);
    if (digitalRead(L_OPEN_BUMP) == HIGH) digitalWrite(L_DOOR_OPEN,OFF);
    if ((digitalRead(R_OPEN_BUMP) == HIGH) && (digitalRead(L_OPEN_BUMP) == HIGH)) break;
  }
}
...
void doorWaitClose(){
  while (1)
  {
    if (digitalRead(CLOSE_BUMP) == HIGH) break;
  }
  digitalWrite(L_DOOR_CLOSE,OFF);
  digitalWrite(R_DOOR_CLOSE,OFF);
}
